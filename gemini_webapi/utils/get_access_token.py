...
async def get_access_token(
    ...
    has_rich_cookies = False
    if isinstance(base_cookies, Cookies):
        non_auth_count = sum(
            1 for c in base_cookies.jar
            if c.name not in ("__Secure-1PSID", "__Secure-1PSIDTS") and not c.is_expired()
        )
        has_rich_cookies = non_auth_count > 0
    elif isinstance(base_cookies, dict):
        non_auth_count = sum(
            1 for k in base_cookies
            if k not in ("__Secure-1PSID", "__Secure-1PSIDTS") and base_cookies[k]
        )
        has_rich_cookies = non_auth_count > 0
            ...
            if content:
                ...
                try:
                    ...
                    for cookie in cookies_data:
                        ...
                        if has_rich_cookies and cookie["name"] not in ("__Secure-1PSID", "__Secure-1PSIDTS"):
                            continue
            ...
            for browser, cookie_list in browser_cookies.items():
                ...
                if secure_1psid:
                    ...
                    if secure_1psidts not in tried_sessions.get(secure_1psid, set()):
                        ...
                        if verbose:
                            logger.debug(
                                f"Prepared full browser cookie jar from {browser}."
                            )
