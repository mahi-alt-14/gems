...
class GeminiClient(ChatMixin, GemMixin, ResearchMixin):
    ...
    SNLM0E_REFRESH_INTERVAL = 39600

    async def start_auto_refresh(self) -> None:
        """
        Start the background task to automatically refresh cookies and SNlM0e token.
        """
        ...
        while self._running:
            ...
            try:
                async with self._lock:
                    saved_cookies_before = {
                        c.name: c.value for c in self._cookies.jar
                    }
                    ...
                    if new_1psidts:
                        ...
                        for c in self.client.cookies.jar:
                            if c.name not in saved_cookies_before and not c.is_expired():
                                self._cookies.set(
                    ...
                    if self._init_time and (
                        time.time() - self._init_time >= self.SNLM0E_REFRESH_INTERVAL
                    ):
                        logger.debug("SNlM0e token approaching 12-hour expiry. Refreshing...")
                        try:
                            response = await self.client.get(
                                Endpoint.INIT, headers=Headers.GEMINI.value
                            )
                            if self.verbose:
                                logger.debug(
                                    f"HTTP Request: GET {Endpoint.INIT} [{response.status_code}]"
                                )
                            ...
                            if access_token:
                                ...
                                if build_label:
                                    self.build_label = build_label.group(1)
                                if session_id:
                                    self.session_id = session_id.group(1)
                                if language:
                                    self.language = language.group(1)
                                if push_id:
                                    self.push_id = push_id.group(1)
                                self._init_time = time.time()
                                self._reqid = random.randint(10000, 99999)
                                logger.success("SNlM0e token refreshed in background.")
                            else:
                                logger.warning(
                                    "SNlM0e token not found in refresh response. "
                                    "Cookies may be fully expired."
                                )
                        except Exception as snl_err:
                            logger.warning(
                                f"Failed to refresh SNlM0e token in background: {snl_err}"
                            )
    ...
    async def refresh_snlm0e(self) -> None:
        """
        Refresh the SNlM0e access token and related session metadata by fetching
        a fresh HTML payload from Gemini. This avoids the need to destroy and
        re-initialize the entire GeminiClient when the SNlM0e token expires
        (typically after ~12 hours).

        Safe to call while the client is running. Acquires the internal lock
        to prevent concurrent request conflicts.

        Raises
        ------
        `gemini_webapi.AuthError`
            If the refresh request fails (e.g. cookies fully expired).
        `RuntimeError`
            If the client is not initialized.
        """

        if not self._running or not self.client:
            raise RuntimeError("Cannot refresh SNlM0e: client is not initialized.")
        ...
        async with self._lock:
            ...
            if not access_token:
                raise AuthError(
                    "Failed to refresh SNlM0e token. The SNlM0e pattern was not found "
                    "in the HTML response. Your cookies may have fully expired."
                )
            ...
            logger.success("SNlM0e token refreshed successfully.")
                    ...
                    if not is_completed or is_thinking or is_queueing:
                        if (
                            ...
                            if is_queueing and not has_candidates:
                                logger.debug(
                                    f"Stream dropped while queueing (completed={is_completed}, final_chunk={is_final_chunk}, "
                                    f"thinking={is_thinking}, queueing={is_queueing}). Session likely flagged. (Request ID: {_reqid})"
                                )
                                raise QueueingDroppedError(
                                    "Stream was dropped due to queueing (bot-flag). "
                                    "This typically indicates the session was flagged by Google's front-end, "
                                    "often due to missing anti-abuse cookies (AEC, __Secure-1PSIDCC, SIDCC, NID) "
                                    "or expired authentication tokens. Supply a full browser cookie jar and retry."
                                )
                            logger.debug(
